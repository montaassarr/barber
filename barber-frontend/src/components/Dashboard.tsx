...
const Dashboard: React.FC<DashboardProps> = ({ salonId: propSalonId, userId: propUserId, userRole = 'owner' }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        // Update existing - just update UI for now on dashboard
        ...
        // Create new - call the actual API
        const newAppointmentInput = {
          salon_id: salonId,
          staff_id: formData.staffId || '',
          service_id: formData.serviceId || '',
          customer_name: formData.customerName || 'New Client',
          customer_email: '',
          customer_phone: '',
          appointment_date: formData.appointmentDate || new Date().toISOString().split('T')[0],
        ...
        const { data, error } = await createAppointment(newAppointmentInput);
        ...
        // Add the created appointment to the list
        if (data) {
  ...
  return (
    <div className="p-3 sm:p-5 md:p-7 lg:p-10 w-full max-w-[1600px] mx-auto space-y-5 sm:space-y-7 md:space-y-9 relative">
          ...
          <div className="bg-white dark:bg-treservi-card-dark rounded-[24px] sm:rounded-[28px] md:rounded-[32px] p-4 sm:p-6 md:p-8 shadow-soft-glow">
            <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-3 sm:gap-0 mb-4 sm:mb-6">
              ...
              <div className="flex items-center gap-2">
                <button onClick={handleAddNew} className="flex items-center justify-center gap-2 bg-treservi-accent hover:bg-green-600 text-white px-3 xs:px-4 sm:px-5 py-2.5 xs:py-2.5 sm:py-2 rounded-full text-xs xs:text-sm sm:text-sm font-bold shadow-neon-glow transition-all transform hover:scale-105 active:scale-95 min-h-[44px] xs:min-h-[44px] sm:min-h-0">
                  <Plus className="w-4 h-4 xs:w-4 xs:h-4 sm:w-4 sm:h-4" /> <span className="hidden sm:inline">{t('appointments.newAppointment')}</span>
                </button>
                <select
                  value={filterStaffId}
      ...
      {/* Appointment Modal - Shared Shell */}
      <AppointmentModal
        isOpen={isModalOpen}
        ...
        formId="dashboard-appointment-form"
        cancelLabel="Cancel"
